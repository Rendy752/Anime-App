// BroadcastNotificationWorker.kt
...
class BroadcastNotificationWorker @AssistedInject constructor(
    ...
    companion object {
        ...
        private const val NOTIFICATION_WINDOW_MINUTES =
            5L // Keep as 5L or change to 15L as per requirement
        ...
        fun schedule(context: Context, forceReschedule: Boolean = false) { //
            ...
            val midnight =
                currentTime.toLocalDate().plusDays(1).atStartOfDay(ZoneId.of("Asia/Jakarta")) //
            val initialDelay = Duration.between(currentTime, midnight).toMillis() //

            val constraints = Constraints.Builder()
                .setRequiredNetworkType(NetworkType.CONNECTED) // Changed to CONNECTED
                .build() //
